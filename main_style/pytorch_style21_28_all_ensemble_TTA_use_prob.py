...
if __name__ == '__main__':
    ...
    CROP_SIZE_LIST1 = [8,10,12,14,16,19]
    CROP_SIZE_LIST2 = [13]
    CHANNELS_1 = 4
    NUM_CLASSES = 3
    BATCH_SIZE = 256
    PER_MODEL_PREDICT_NUM = 1
    ...
    for test_csv in [style_public_test_path, style_private_test_path]:
        ...
        for pred_id in tqdm.tqdm(keep_dict):
            ...
            df.to_csv(f'{public_test_path}/Style21_private_{[x for x in CROP_SIZE_LIST1]}_style28_{[x for x in CROP_SIZE_LIST2]}_TTA_{PER_MODEL_PREDICT_NUM}_use_prob.csv', mode='a', index=False, header=False)
