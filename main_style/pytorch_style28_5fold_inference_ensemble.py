    ...
    for test_csv in [style_public_test_path, style_private_test_path]:
        ...
        for CROP_SIZE in CROP_SIZE_LIST:
            for per_fold in range(5):
                ...
                MODEL_NAME = f'Style_model28_channel{CHANNELS_1}_{CROP_SIZE}_fold{FOLD}_valloss_best'
        ...
        for idx, per_pred_id in enumerate(pred_id):
            ...
            df.to_csv(f'{public_test_path}/Style_model28_5fold_valloss_ensemble_{[x for x in CROP_SIZE_LIST]}.csv', mode='a', index=False, header=False)
