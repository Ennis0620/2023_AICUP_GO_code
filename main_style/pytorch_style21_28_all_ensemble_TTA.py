...
if __name__ == '__main__':
    ...
    for test_csv in [style_public_test_path, style_private_test_path]:
        ...
        all_pred_id = []
        # 每個model要predict多少次
        for CROP_SIZE in CROP_SIZE_LIST2:
            for per_fold in range(5):
                ...
                MODEL_NAME = f'Style_model28_5fold_channel{CHANNELS_1}_{CROP_SIZE}_fold{FOLD}_valloss_best'
        ...
        for CROP_SIZE in CROP_SIZE_LIST1:
        ...
        for idx, per_pred_id in enumerate(pred_id):
            ...
            # most_occur = np.argmax(occur_count)#取得出現次出最多的那個index

            df = pd.DataFrame({'name' : per_pred_id, '0' : int(most_occur+1)}, index=[0])
            df.to_csv(f'{public_test_path}/Style21_private_{[x for x in CROP_SIZE_LIST1]}_style28_{[x for x in CROP_SIZE_LIST2]}_TTA_{PER_MODEL_PREDICT_NUM}.csv', mode='a', index=False, header=False)
