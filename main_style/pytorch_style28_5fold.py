...
from utils.preprocess import prepare_board_status_BW_inverse_both, prepare_board_status, augmentation_crop_aug
...
class Go_Dataset_aug(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.data_type == 'train':
            ...
            styles = self.train_game_styles[index]
            target_color = moves_list[-1][0]#最後一手是哪個顏色下的
            _,x,_ = prepare_board_status_BW_inverse_both(moves_list,
                                        sperate_posotion=True,
                                        empty_is_one=False,
                                        last_move_ = True,
                                        crop_size = self.crop_size)
            x = augmentation_crop_aug(x)
            y = [styles-1 for x in range(8*4)]
            x = np.array(x)
            y = np.array(y)
            return x, y
...
if __name__ == '__main__': 
    ...
    for per_fold in range(5):
        ...
        MODEL_NAME = f'Style_model28_5fold_channel{CHANNELS_1}_{CROP_SIZE}_fold{FOLD}_valloss'
        ...
        Style_train_dataloader = Go_Dataset_aug(csv_path=Style_csv_path, data_type='train', crop_size=CROP_SIZE)
        Style_valid_dataloader = Go_Dataset(csv_path=Style_csv_path, data_type='valid', crop_size=CROP_SIZE, )
        Style_train_dataloader = DataLoader(Style_train_dataloader, batch_size=BATCH_SIZE, shuffle=True, num_workers=4)
