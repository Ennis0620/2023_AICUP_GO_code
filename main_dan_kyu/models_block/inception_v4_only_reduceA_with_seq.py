...
class Inception_Go_seqence(nn.Module):
    ...
    def __init__(self, version, in_channels ,num_classes, is_se=False):
        ...
        self.seq = nn.Sequential(
            ...
            nn.Conv3d(64,32,kernel_size=(1,1,1),bias=False),#input處理成(B,1,channel=8,19,19) conv3d後 變成 (B,channel,1,19,19)
            nn.InstanceNorm3d(32),
            nn.ReLU(inplace=True),
            nn.Conv3d(32,1,kernel_size=(1,1,1),bias=False),#input處理成(B,1,channel=8,19,19) conv3d後 變成 (B,channel,1,19,19)
        )
        self.GAP = nn.AdaptiveAvgPool2d(1)
        self.GMP = nn.AdaptiveMaxPool2d(1)

        #可學習的權重矩陣
        self.weights_B = nn.Parameter(torch.randn(1),requires_grad=True)
        self.weights_W = nn.Parameter(torch.randn(1),requires_grad=True)
    ...
    def forward(self, x, seq):
        #sequence處理
        seq_split = torch.split(seq, 8, dim=2)
        seq_B = self.seq(seq_split[0])
        seq_B = seq_B.permute(0,2,1,3,4)
        seq_B = seq_B.view(-1,seq_B.size(2),seq_B.size(3),seq_B.size(4))
        seq_W = self.seq(seq_split[1])
        seq_W = seq_W.permute(0,2,1,3,4)
        seq_W = seq_W.view(-1,seq_W.size(2),seq_W.size(3),seq_W.size(4))
        
        x = x * (seq_B*self.weights_B) * (seq_W *self.weights_W)
...
class Inception_Go_seqence_V2(nn.Module):
    """
    Inception-v4, Inception-ResNet-v1, Inception-ResNet-v2
    """

    def __init__(self, version, in_channels ,num_classes, is_se=False):
        super(Inception_Go_seqence_V2, self).__init__()
        self.version = version
        self.inception_A = self.__make_inception_A(in_channels)
        self.Reduction_A = self.__make_reduction_A()
        self.inception_B = self.__make_inception_B()
    ...
    def forward(self, x, seq):
        ...
        x[:,0,:,:] *= seq_B[:,0,:,:] * self.weights
        x[:,1,:,:] *= seq_W[:,0,:,:] * (1-self.weights)
...
class Inception_Go_seqence_V2_V2(nn.Module):
    """
    剃除 instance normalize、torch.rand
    """

    def __init__(self, version, in_channels ,num_classes, is_se=False):
        super(Inception_Go_seqence_V2_V2, self).__init__()
        self.version = version
        self.inception_A = self.__make_inception_A(in_channels)
        self.Reduction_A = self.__make_reduction_A()
        self.inception_B = self.__make_inception_B()
...
class Inception_Go_seqence_V3(nn.Module):
    """
    Inception-v4, Inception-ResNet-v1, Inception-ResNet-v2
    """

    def __init__(self, version, in_channels ,num_classes, is_se=False):
        super(Inception_Go_seqence_V3, self).__init__()
        self.version = version
        self.inception_A = self.__make_inception_A(in_channels)
        self.Reduction_A = self.__make_reduction_A()
        self.inception_B = self.__make_inception_B()
...
class Inception_Go_seqence_V3_V2(nn.Module):
    """
    Inception-v4, Inception-ResNet-v1, Inception-ResNet-v2
    """

    def __init__(self, version, in_channels ,num_classes, is_se=False):
        super(Inception_Go_seqence_V3_V2, self).__init__()
        self.version = version
        self.inception_A = self.__make_inception_A(in_channels)
        self.Reduction_A = self.__make_reduction_A()
        self.inception_B = self.__make_inception_B()
...
class Inception_Go_seqence_V3_no_Seq(nn.Module):
    """
    Inception-v4, Inception-ResNet-v1, Inception-ResNet-v2
    """

    def __init__(self, version, in_channels ,num_classes, is_se=False):
        super(Inception_Go_seqence_V3_no_Seq, self).__init__()
        self.version = version
        self.inception_A = self.__make_inception_A(in_channels)
        self.Reduction_A = self.__make_reduction_A()
        self.inception_B = self.__make_inception_B()
...
def inception_v4_GO(in_channels, num_classes=361):
    return Inception_Go_seqence("v4",in_channels, num_classes)

def inception_resnet_v1_GO(in_channels, num_classes=361):
    return Inception_Go_seqence("res1",in_channels, num_classes)

def inception_resnet_v2_GO(in_channels, num_classes=361):
    return Inception_Go_seqence("res2",in_channels, num_classes)

def inception_v4_GO_V2(in_channels, num_classes=361):
    return Inception_Go_seqence_V2("v4",in_channels, num_classes)

def inception_resnet_v1_GO_V2(in_channels, num_classes=361):
    return Inception_Go_seqence_V2("res1",in_channels, num_classes)

def inception_resnet_v2_GO_V2(in_channels, num_classes=361):
    return Inception_Go_seqence_V2("res2",in_channels, num_classes)

def inception_v4_GO_V2_V2(in_channels, num_classes=361):
    return Inception_Go_seqence_V2_V2("v4",in_channels, num_classes)

def inception_resnet_v1_GO_V2_V2(in_channels, num_classes=361):
    return Inception_Go_seqence_V2_V2("res1",in_channels, num_classes)

def inception_resnet_v2_GO_V2_V2(in_channels, num_classes=361):
    return Inception_Go_seqence_V2_V2("res2",in_channels, num_classes)

def inception_v4_GO_V3(in_channels, num_classes=361):
    return Inception_Go_seqence_V3("v4",in_channels, num_classes)

def inception_resnet_v1_GO_V3(in_channels, num_classes=361):
    return Inception_Go_seqence_V3("res1",in_channels, num_classes)

def inception_resnet_v2_GO_V3(in_channels, num_classes=361):
    return Inception_Go_seqence_V3("res2",in_channels, num_classes)

def inception_v4_GO_V3_V2(in_channels, num_classes=361):
    return Inception_Go_seqence_V3_V2("v4",in_channels, num_classes)

def inception_resnet_v1_GO_V3_V2(in_channels, num_classes=361):
    return Inception_Go_seqence_V3_V2("res1",in_channels, num_classes)

def inception_resnet_v2_GO_V3_V2(in_channels, num_classes=361):
    return Inception_Go_seqence_V3_V2("res2",in_channels, num_classes)

def inception_v4_GO_V3_no_Seq(in_channels, num_classes=361):
    return Inception_Go_seqence_V3_no_Seq("v4",in_channels, num_classes)

def inception_resnet_v1_GO_V3_no_Seq(in_channels, num_classes=361):
    return Inception_Go_seqence_V3_no_Seq("res1",in_channels, num_classes)

def inception_resnet_v2_GO_V3_no_Seq(in_channels, num_classes=361):
    return Inception_Go_seqence_V3_no_Seq("res2",in_channels, num_classes)
...
if __name__ == '__main__':
    ...
    model = inception_resnet_v2_GO_V3_V2(in_channels = CHANNELS_1, num_classes = NUM_CLASSES)
    model = model.to(DEVICE)
    summary(model, torch.randn(2, CHANNELS_1, CROP_SIZE, CROP_SIZE).to(DEVICE),torch.randn(2, 1, CHANNELS_2, CROP_SIZE, CROP_SIZE).to(DEVICE))
